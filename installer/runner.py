...
def refresh_assets(ctx: InstallerContext) -> int:
    ...
    if "all" in targets:
        targets = {"firefox", "chrome", "obsidian", "desktop"}
    ...
    if "chrome" in targets:
        chrome.seed_chrome(ctx)
        record_result(ctx, "refresh-chrome", "ok", "Reapplied Chrome assets")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--refresh-assets",
        nargs="+",
        choices=["firefox", "chrome", "obsidian", "desktop", "all"],
    )
    parser.add_argument(
        "--only",
        dest="only_components",
        action="append",
        choices=[component.id for component in DEFAULT_COMPONENTS],
        default=[],
    )
...
def resolve_components(args: argparse.Namespace) -> list[str]:
    if args.only_components:
        return list(dict.fromkeys(args.only_components))
...
def installer_exit_code(args: argparse.Namespace, ctx: InstallerContext) -> int:
    if args.only_components:
        return 1 if any(result.status == "failed" for result in ctx.results) else 0
    return 1 if any(result.fatal and result.status == "failed" for result in ctx.results) else 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    return installer_exit_code(args, ctx)
